...
/**
 * Regenerates {@code docs/api/pic-sure-operations-service.openapi.json} from the running context and asserts the contract invariants that
...
class OpenApiExportTest {
    ...
    private static final String ARTIFACT = "pic-sure-operations-service";
    ...
    void exportsTheDocumentAndHoldsTheSharedContractInvariants() throws Exception {
        ...
        // Representative shape: the internal query store hands back the shared StoredQuery record, not a JPA entity --
        // this service owns the pic-sure DB and the entity must never reach the wire.
        OpenApiExport.assertRespondsWith(
            OpenApiExport.operation(document, "/internal/queries/{picsureId}", "get"), "application/json", "StoredQuery"
        );
    }
}
