...
public class Loader {
	...
	private static byte[] key = "ANENCRYPTIONKEY".getBytes();
			...
			.removalListener(new RemovalListener<String, PhenoCube>() {
				...
				private <V extends Comparable<V>> void complete(PhenoCube<V> cube) {
					...
					if(cube.isStringType()) {
						...
						log.info("setting category map for " + cube.name + " : " + categorySetMap);
	...
	private static void initialLoad() throws IOException {
		...
		for (CSVRecord record : records) {
			...
			try {
				...
				if(value != null && ((isAlpha && currentConcept[0].vType == String.class)||(!isAlpha && currentConcept[0].vType == Float.class))) {
					currentConcept[0].setColumnWidth(isAlpha ? Math.max(currentConcept[0].getColumnWidth(), value.getBytes().length) : Float.BYTES);
					currentConcept[0].add(Integer.parseInt(record.get(PATIENT_NUM)), isAlpha ? value : Float.parseFloat(value));
				}
