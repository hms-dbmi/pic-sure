...
public class GenomicProcessorPatientMergingParentImplTest {
    ...
    public void getPatientMask_validResponses_returnMerged() {
        ...
        when(mockProcessor1.getPatientMask(distributableQuery)).thenReturn(Mono.just(new BigInteger("11011011", 2)));
        when(mockProcessor2.getPatientMask(distributableQuery)).thenReturn(Mono.just(new BigInteger("110001100011", 2)));
        when(mockProcessor3.getPatientMask(distributableQuery)).thenReturn(Mono.just(new BigInteger("11000111", 2)));
        BigInteger patientMask = patientMergingParent.getPatientMask(distributableQuery).block();
        BigInteger expectedPatientMask = new BigInteger("11011000011000000111", 2);
    ...
    public void getPatientMask_noPatientResponses_returnMerged() {
        DistributableQuery distributableQuery = new DistributableQuery();
        when(mockProcessor1.getPatientMask(distributableQuery)).thenReturn(Mono.just(new BigInteger("11011011", 2)));
        when(mockProcessor2.getPatientMask(distributableQuery)).thenReturn(Mono.just(new BigInteger("110000000011", 2)));
        when(mockProcessor3.getPatientMask(distributableQuery)).thenReturn(Mono.just(new BigInteger("11000011", 2)));
        BigInteger patientMask = patientMergingParent.getPatientMask(distributableQuery).block();
        BigInteger expectedPatientMask = new BigInteger("11011000000000000011", 2);
        assertEquals(expectedPatientMask, patientMask);
    }
    ...
    public void getPatientMask_emptyResponses_returnMerged() {
        DistributableQuery distributableQuery = new DistributableQuery();
        when(mockProcessor1.getPatientMask(distributableQuery)).thenReturn(Mono.just(new BigInteger("11011011", 2)));
        when(mockProcessor2.getPatientMask(distributableQuery)).thenReturn(Mono.just(new BigInteger("1111", 2)));
        when(mockProcessor3.getPatientMask(distributableQuery)).thenReturn(Mono.just(new BigInteger("11000111", 2)));
        BigInteger patientMask = patientMergingParent.getPatientMask(distributableQuery).block();
        BigInteger expectedPatientMask = new BigInteger("110110000111", 2);
        assertEquals(expectedPatientMask, patientMask);
    }
