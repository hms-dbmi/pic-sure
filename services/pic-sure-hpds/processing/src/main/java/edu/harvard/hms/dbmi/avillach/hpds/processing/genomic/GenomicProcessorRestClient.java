...
public class GenomicProcessorRestClient implements GenomicProcessor {
    ...
    private static final ParameterizedTypeReference<List<InfoColumnMeta>> INFO_COLUMNS_META_TYPE_REFERENCE =
    ...
    public GenomicProcessorRestClient(String serviceUrl) {
        this.webClient =
            WebClient.builder().baseUrl(serviceUrl).defaultHeader(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE).build();
    }

    @Override
    public Mono<VariantMask> getPatientMask(DistributableQuery distributableQuery) {
        Mono<VariantMask> result = webClient.post().uri("/patients").contentType(MediaType.APPLICATION_JSON)
            .body(Mono.just(distributableQuery), DistributableQuery.class).retrieve().bodyToMono(VariantMask.class);
        return result;
    }
    ...
    public Mono<Set<String>> getVariantList(DistributableQuery distributableQuery) {
        Mono<Set<String>> result = webClient.post().uri("/variants").contentType(MediaType.APPLICATION_JSON)
            .body(Mono.just(distributableQuery), DistributableQuery.class).retrieve().bodyToMono(VARIANT_SET_TYPE_REFERENCE);
        return result;
    }

    @Override
    public List<String> getPatientIds() {
        List<String> result = webClient.get().uri("/patients/ids").retrieve().bodyToMono(LIST_OF_STRING_TYPE_REFERENCE).block();
        return result;
    }
    ...
    public Set<String> getInfoStoreColumns() {
        Set<String> result = webClient.get().uri(uriBuilder -> uriBuilder.path("/info/columns").build()).retrieve()
            .bodyToMono(SET_OF_STRING_TYPE_REFERENCE).block();
        return result;
    }
    ...
    public Set<String> getInfoStoreValues(String conceptPath) {
        Set<String> result =
            webClient.get().uri(uriBuilder -> uriBuilder.path("/info/values").queryParam("conceptPath", conceptPath).build(conceptPath))
                .retrieve().bodyToMono(SET_OF_STRING_TYPE_REFERENCE).block();
        return result;
    }

    @Override
    public List<InfoColumnMeta> getInfoColumnMeta() {
        List<InfoColumnMeta> result = webClient.get().uri("/info/meta").retrieve().bodyToMono(INFO_COLUMNS_META_TYPE_REFERENCE).block();
        return result;
    }
