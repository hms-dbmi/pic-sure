...
public class AbstractProcessor {
    ...
    public AbstractProcessor(
        ...
        @Value("${HPDS_DATA_DIRECTORY:/opt/local/hpds/}") String hpdsDataDirectory, @Value("${CACHE_SIZE:100}") int cacheSize
        ...
        if (Crypto.hasKey(Crypto.DEFAULT_KEY_NAME)) {
            ...
            int conceptsToCache = Math.min(cubes.size(), this.cacheSize);
    ...
    public AbstractProcessor(
        ...
        String hpdsDataDirectory, @Value("${CACHE_SIZE:100}") int cacheSize
    ) {
        this.phenotypeMetaStore = phenotypeMetaStore;
        this.store = store;
        this.genomicProcessor = genomicProcessor;
    ...
    protected LoadingCache<String, PhenoCube<?>> initializeCache() {
        return CacheBuilder.newBuilder().maximumSize(cacheSize).build(new CacheLoader<String, PhenoCube<?>>() {
