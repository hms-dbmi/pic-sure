...
public class LoadingStore {
    ...
    public LoadingCache<String, PhenoCube> store =
        CacheBuilder.newBuilder().maximumSize(16).removalListener(new RemovalListener<String, PhenoCube>() {
            ...
            public void onRemoval(RemovalNotification<String, PhenoCube> arg0) {
                ...
                try {
                    ColumnMeta columnMeta = new ColumnMeta().setName(arg0.getKey()).setWidthInBytes(arg0.getValue().getColumnWidth())
                        .setCategorical(arg0.getValue().isStringType());
                    ...
                    columnMeta.setPatientCount(Arrays.stream(arg0.getValue().sortedByKey()).map((kv) -> {
                        return kv.getKey();
                    ...
                    if (columnMeta.isCategorical()) {
                        ...
                        List<Double> map = (List<Double>) arg0.getValue().keyBasedArray().stream().map((value) -> {
                            return (Double) value;
    ...
    public void saveStore(String hpdsDirectory) {
        try {
    ...
    public void dumpStats() {
        ...
        try (
            ObjectInputStream objectInputStream =
                new ObjectInputStream(new GZIPInputStream(new FileInputStream("/opt/local/hpds/columnMeta.javabin")));
        ) {
            ...
            for (String key : metastore.keySet()) {
                ...
                System.out.println(
                    String.join(
                        "\t", key.toString(), columnMeta.getObservationCount() + "",
                        columnMeta.getMin() == null ? "NaN" : columnMeta.getMin().toString(),
                        columnMeta.getMax() == null ? "NaN" : columnMeta.getMax().toString(),
                        columnMeta.getCategoryValues() == null ? "NUMERIC CONCEPT"
                            : String.join(",", columnMeta.getCategoryValues().stream().map((value) -> {
    ...
    /**
     * This method will display counts for the objects stored in the metadata. This will also write out a csv file used by the data
     * dictionary importer.
     */
