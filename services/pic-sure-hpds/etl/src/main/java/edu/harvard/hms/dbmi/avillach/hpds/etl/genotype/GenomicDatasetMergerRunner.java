...
public class GenomicDatasetMergerRunner {
    ...
    /**
     * args[0]: directory containing genomic dataset 1 args[1]: directory containing genomic dataset 2 args[2]: output directory
     */
    public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
        if (args.length != 3) {
            throw new IllegalArgumentException(
                "Three arguments must be provided: source directory 1, source directory 2, output directory"
            );
        }
        ...
        GenomicDatasetMerger genomicDatasetMerger = new GenomicDatasetMerger(
            VariantStore.readInstance(genomicDirectory1), VariantStore.readInstance(genomicDirectory2), infoStores1, infoStores2,
            outputDirectory
        );
        ...
        Map<String, FileBackedJsonIndexStorage<Integer, ConcurrentHashMap<String, VariableVariantMasks>>> mergedChromosomeMasks =
            genomicDatasetMerger.mergeChromosomeMasks();
    ...
    private static Map<String, FileBackedByteIndexedInfoStore> loadInfoStores(String directory) {
        ...
        if (genomicDataDirectory.exists() && genomicDataDirectory.isDirectory()) {
            Arrays.stream(genomicDataDirectory.list((file, filename) -> {
                return filename.endsWith(INFO_STORE_JAVABIN_SUFFIX);
            })).forEach((String filename) -> {
                try (
                    FileInputStream fis = new FileInputStream(directory + filename); GZIPInputStream gis =
