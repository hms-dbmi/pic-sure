...
/**
 * 
 * This loader will read in the metadata associated with each variant and build a VariantMetadataIndex that can be used to populate data in
 * the PIC-SURE Varaint Explorer.
 */
public class SplitChromosomeVariantMetadataLoader {
    ...
    private static final int ANNOTATED_FLAG_COLUMN = 2, GZIP_FLAG_COLUMN = 3, FILE_COLUMN = 0;
    ...
    private static void createVariantMetadataIndexForContig(VcfInputFile vcfInput) {
        try {
            ...
            VariantMetadataIndex metadataIndex =
                new VariantMetadataIndex(hpdsDataPath + contig + "/" + VARIANT_METADATA_STORAGE_FILE_PREFIX);
