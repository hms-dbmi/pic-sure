...
public class RemapIds {
        ...
        for (String conceptPath : sourceMetaStore.keySet()) {
            ...
            for (KeyAndValue entry : cubeForPath.sortedByKey()) {
                processRecord(
                    currentConcept,
                    List.of(
                        entry.getKey().toString(), conceptPath, cubeForPath.isStringType() ? "" : entry.getValue().toString(),
    ...
    private static void processRecord(final PhenoCube[] currentConcept, List<String> record) {
        try {
            ...
            if (
                ...
                currentConcept[0].add(
                    patientId, isAlpha ? value : Double.parseDouble(value),
                    record.get(TIMESTAMP) == null ? null : new Date(Long.parseLong(record.get(TIMESTAMP)))
                );
     ...
     * @throws IOException
     */
    protected static LoadingCache<String, PhenoCube<?>> initializeCache()
        throws ClassNotFoundException, FileNotFoundException, IOException {
        return CacheBuilder.newBuilder().maximumSize(10).build(new CacheLoader<String, PhenoCube<?>>() {
    ...
    protected static Object[] loadMetadata() {
        try (
            ObjectInputStream objectInputStream =
                new ObjectInputStream(new GZIPInputStream(new FileInputStream("/opt/local/source/columnMeta.javabin")));
        ) {
            ...
            log.warn(
                "If you meant to include phenotype data of any kind, please check that the file /opt/local/source/columnMeta.javabin exists and is readable by the service."
            );
            log.warn("************************************************");
            log.warn("************************************************");
