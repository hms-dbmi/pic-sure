...
/**
 * This class provides authentication functionality. This implements an authenticationService interface in the future to support different
 * modes of authentication.
 *
 * <h3>Thoughts of design</h3> The main purpose of this class is returns a token that includes information of the roles of users.
 */
...
public class Auth0AuthenticationService implements AuthenticationService {
    ...
    public Auth0AuthenticationService(
        OauthUserMatchingService matchingService, UserRepository userRepository, BasicMailService basicMailService, UserService userService,
        ConnectionRepository connectionRepository, RestClientUtil restClientUtil,
        @Value("${auth0.idp.provider.is.enabled}") boolean isAuth0Enabled,
        @Value("${auth0.denied.email.enabled}") boolean deniedEmailEnabled, @Value("${auth0.host}") String auth0host,
    ...
    @Override
    public AuthenticationResponse authenticate(AuthenticationRequest authRequest, String requestHost) throws IOException {
        ...
        Connection connection = connectionRepository.findById(connectionId)
            .orElseThrow(() -> new NotAuthorizedException("No connection found for connection_id " + connectionId));
        ...
        for (int i = 1; i <= AUTH_RETRY_LIMIT && auth0Response == null; i++) {
            try {
                ResponseEntity<String> response =
                    this.restClientUtil.retrieveGetResponseWithRequestConfiguration(auth0UserInfoURI, headers, 2000);
