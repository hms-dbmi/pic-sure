...
class GatewaySecurityPropertiesAioBindingTest {
    ...
    /**
     * Regression: this must NOT fall back to the empty string when {@code OPERATIONS_SERVICE_URL} is unset. {@code QueryAuthFetcher} builds
     * {@code {operationsServiceUrl}/operations/internal/queries/{id}/dispatch} to authorize the bodyless {@code /query/{id}/result} and
     * {@code /signed-url} reads. An empty base leaves a RELATIVE uri, and the {@code IllegalArgumentException("URI with undefined scheme")}
     * that follows is not a {@code RestClientException} -- so it escapes {@code QueryAuthFetcher}'s fail-closed catches AND the gateway's
     * {@code PicsureException}-only advice, surfacing as a bare Boot 500 on downloads while every other endpoint keeps working.
     */
    @Test
