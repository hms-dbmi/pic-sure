 ...
 * {@code publicBase} is the ingress prefix the per-service public prefixes are built from ({@code GATEWAY_DOCS_PUBLIC_BASE}); each entry of
 * {@code services} is one proxied document. Duplicate names fail binding, and so startup.
 */
@ConfigurationProperties(prefix = "picsure.gateway.docs")
public record DocsProperties(Boolean enabled, String publicBase, List<DocumentedService> services) {
