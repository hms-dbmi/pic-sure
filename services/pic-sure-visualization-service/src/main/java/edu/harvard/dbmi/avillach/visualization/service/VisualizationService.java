...
public class VisualizationService {
    ...
    public VisualizationService(
        ...
        HpdsClient hpdsClient,
        ObfuscationParser obfuscationParser,
        CategoricalDistributionProcessor categoricalDistributionProcessor,
        ContinuousDistributionProcessor continuousDistributionProcessor,
        ...
        this.categoricalDistributionProcessor =
            categoricalDistributionProcessor;
    ...
    public VisualizationResponse generateDistributions(
        Query query,
        HpdsAccessContext accessContext,
        String bearerToken
    ) {
    ...
    public VisualizationResponse generateDistributions(
        ...
        List<QueryDecomposer.SubQueryDescriptor> subQueries =
            ...
            try {
                if (accessContext.accessType() == AccessType.AUTHORIZED) {
                    Map<String, Map<String, Integer>> crossCounts =
                        requestId == null
                            ...
                            : hpdsClient.getAuthCrossCounts(
                                  ...
                                  descriptor.distributionKind()
                              );
                    addDistributions(
                        descriptor,
                        crossCounts,
                        DistributionProcessingOptions.AUTHORIZED,
                        false,
                        categoricalData,
                        continuousData
                    );
                    ...
                    if (hasSeriesData(rawCrossCounts)) {
                        boolean isObfuscated = obfuscationParser.isObfuscated(
                            rawCrossCounts
                        );
                        Map<String, Map<String, Integer>> cleanedCounts =
                            obfuscationParser.clean(rawCrossCounts);
                        if (hasSeriesData(cleanedCounts)) {
                            addDistributions(
                                descriptor,
                                cleanedCounts,
                                DistributionProcessingOptions.OPEN,
                                isObfuscated,
                                categoricalData,
                                continuousData
                            );
                ...
                logger.error(
                    "HPDS returned HTTP {} for {} {} query",
                    e.getStatusCode().value(),
                    accessContext.accessType().getValue(),
                    descriptor.resultType(),
                    e
                );
                throw new HpdsUpstreamException(
                    "HPDS query failed with status " +
                        e.getStatusCode().value() +
                        ": " +
                        e.getStatusText(),
                    e
                ...
                logger.error(
                    "Could not reach HPDS for {} {} query",
                    accessContext.accessType().getValue(),
                    descriptor.resultType(),
                    e
                );
                throw new HpdsUpstreamException(
                    "HPDS query failed: " + e.getMessage(),
                    e
                );
    ...
    private void addDistributions(
        QueryDecomposer.SubQueryDescriptor descriptor,
        Map<String, Map<String, Integer>> crossCounts,
        DistributionProcessingOptions options,
        ...
        if (descriptor.distributionKind() == DistributionType.CATEGORICAL) {
            categoricalData.addAll(
                categoricalDistributionProcessor.process(
                    crossCounts,
                    isObfuscated,
                    options.aggregateCategoricalValues()
                )
            );
        } else {
            continuousData.addAll(
                continuousDistributionProcessor.process(
                    crossCounts,
                    isObfuscated,
                    options.binContinuousValues()
                )
            );
        }
    }

    private static boolean hasSeriesData(
        Map<String, ? extends Map<?, ?>> crossCounts
    ) {
        return (
            crossCounts != null &&
            crossCounts
