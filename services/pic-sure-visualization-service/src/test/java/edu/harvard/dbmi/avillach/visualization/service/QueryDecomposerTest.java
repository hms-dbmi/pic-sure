...
class QueryDecomposerTest {
    ...
    void decompose_withCategoricalFilter_returnsCategoricalSubQuery() {
        ...
        List<QueryDecomposer.SubQueryDescriptor> result = decomposer.decompose(
            query
        );
        ...
        assertEquals(
            ...
            result.get(0).resultType()
        );
        assertEquals(
            DistributionType.CATEGORICAL,
            result.get(0).distributionKind()
        );
        assertEquals(
            List.of("\\demographics\\race\\"),
            result.get(0).query().select()
        );
    }
    ...
    void decompose_withBothFilterTypes_returnsTwoSubQueries() {
        ...
        PhenotypicSubquery subquery = new PhenotypicSubquery(
            null,
            List.of(catFilter, numFilter),
            Operator.AND
        );
        Query query = new Query(
            List.of(),
    ...
    void decompose_withSelectButNoFilters_fallsBackToSelectAsCategorical() {
        Query query = new Query(
            List.of("\\demographics\\race\\", "\\demographics\\sex\\"),
            List.of(),
            null,
            List.of(),
            null,
