...
class AWSClientBuilderTest {
    ...
    void shouldCreateCredentialsWithoutAssumingRole() {
        ...
        Mockito.when(s3ClientBuilder.build()).thenReturn(s3Client);
    ...
    void shouldNotBuildClientIfSiteDNE() {
        Mockito.when(sites.get("Narnia")).thenReturn(null);
    ...
    void shouldNotBuildClientIfRoleRequestFails() {
        ...
        Mockito.when(sites.get("bch")).thenReturn(siteAWSInfo);
        Mockito.when(sites.containsKey("bch")).thenReturn(true);

        ArgumentMatcher<AssumeRoleRequest> requestMatcher = (r) -> r.roleArn().equals("aws:arn:420")
            && r.roleSessionName().startsWith("test_session") && r.externalId().equals("external") && r.durationSeconds().equals(3600);
        AssumeRoleResponse response = Mockito.mock(AssumeRoleResponse.class);
        Mockito.when(stsClient.assumeRole(Mockito.argThat(requestMatcher))).thenReturn(response);
        Mockito.when(stsClientProvider.createClient()).thenReturn(Optional.of(stsClient));
    ...
    void shouldBuildClient() {
        ...
        Mockito.when(assumeRoleResponse.credentials()).thenReturn(credentials);
        ...
        ArgumentMatcher<AssumeRoleRequest> requestMatcher = (r) -> r.roleArn().equals("aws:arn:420")
            && r.roleSessionName().startsWith("test_session") && r.externalId().equals("external") && r.durationSeconds().equals(3600);
        Mockito.when(stsClient.assumeRole(Mockito.argThat(requestMatcher))).thenReturn(assumeRoleResponse);
        Mockito.when(stsClientProvider.createClient()).thenReturn(Optional.of(stsClient));
        ...
        S3Client s3Client = Mockito.mock(S3Client.class);
        Mockito.when(s3ClientBuilder.credentialsProvider(Mockito.argThat(credsMatcher))).thenReturn(s3ClientBuilder);
        Mockito.when(s3ClientBuilder.build()).thenReturn(s3Client);
