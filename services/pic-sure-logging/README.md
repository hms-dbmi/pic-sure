...
Accepts an audit event, enriches it with JWT claims and platform config, and submits it to Logback for routing to stdout and the audit log file.
...
| Status | Condition |
|---|---|
| `202 Accepted` | Event assembled and submitted to Logback; durable file writing and downstream shipping are not guaranteed |
...
**Startup validation:** The service fails fast with a clear error message if `LOGGING_API_KEY` is missing, `PORT` is not a valid integer in range 1-65535, or `JWT_CLAIM_MAPPING` is invalid. Mapping input claims and output fields must be non-blank, output fields must be unique and non-reserved, and `logged_in` is reserved as both an input claim and an output field.
...
body, and canonical `logged_in` is emitted unconditionally by `JwtDecodeService`.
Custom mappings cannot use `logged_in` as either an input claim or an output field.
...
- **`logged_in`** -- Canonical boolean generated by the service (`true` for a decoded token, `false` when no token can be decoded)
...
└── src/main/java/edu/harvard/dbmi/avillach/logging/
    ...
    └── service/
        ├── AuditLogService.java        # Core logic: assemble fields, submit to Logback
        ├── AuditAppenderFailureMonitor.java # Logback appender error telemetry
...
- Synchronous assembly/submission failures return a sanitized `500`; appender failures reported internally by Logback cannot change the response
...
File appenders use `SizeAndTimeBasedRollingPolicy` (rotates daily and at 50 MB, 30-day retention, 1 GB total cap for audit / 500 MB for app) and are wrapped in bounded async appenders. Audit file writes apply backpressure when the 1024-entry queue fills (`neverBlock=false`, `discardingThreshold=0`), while application logging remains non-blocking and best-effort (`neverBlock=true`).
...
The following Micrometer counters are registered at startup. Expose the `prometheus` Actuator endpoint only on a trusted network or behind an authenticating proxy, as described under Configuration.

| Metric | Meaning |
|---|---|
| `picsure.audit.submitted` | Events successfully assembled and submitted to Logback. This does not prove that an appender durably wrote or shipped the event. |
| `picsure.audit.submission.failed` | Synchronous exceptions during audit assembly or submission. These failures propagate as a sanitized HTTP `500`. Logback appender errors are caught internally and do not increment this counter. |
| `picsure.audit.appender.failed` | Logback `ERROR` statuses originating from `AUDIT_JSON`, `AUDIT_FILE`, or `ASYNC_AUDIT_FILE`. Logback may rate-limit repeated status reporting, so this is an alert signal rather than an exact count of lost events. |

The appender-failure listener increments its counter directly and does not write another log event, avoiding recursive Logback failures. An appender error cannot retroactively change an HTTP response, including a `202` already returned for successful submission.
...
built into `target/`. Its multi-architecture Corretto 25 base image is pinned by
digest, and it runs as the fixed numeric UID/GID `10001:10001`.

The container creates `/app/logs` with that ownership. When bind-mounting a host
directory there (including the Compose `./logs:/app/logs` mount), ensure the host
directory is writable by UID/GID `10001:10001`; Docker does not change the
ownership of bind-mounted files.
...
| Audit submission | Synchronous exception during assembly or submission | Increment `picsure.audit.submission.failed`, log to stderr, return sanitized `500 Internal Server Error` |
| Audit appender | Logback reports an error from an audit appender | Increment `picsure.audit.appender.failed`; response remains based on submission and may already be `202` |
...
- **Container user** -- The image runs as numeric UID/GID `10001:10001`. Any host
  directory bind-mounted at `/app/logs` must be writable by that identity.
